            ...
            row.innerHTML =`
                <td >
                    <div class="img-container">
                        <img src="${phoneList[keyName].image}" alt="" class="img">
                    </div>
                    <p>${phoneList[keyName].name}</p>
                </td>
                <td class="text-center" style="padding-top:25px;">
                    <span>${formatter.format(phoneList[keyName].cost_after)}</span>
                    <div class="discount"><i>${formatter.format(phoneList[keyName].cost_before)}</i></div>
                </td>
                <td class="cart-number text-center">
                    ...
                    <span>${parseInt(window.localStorage.getItem(keyName))}</span>
                ...
                <td class="text-center" style="padding-top:25px;">
                    ...
                    <div class="discount"><i>${formatter.format(current*price2)}</i></div>
                </td>
...
function cartNumbers(phoneList){
    ...
    if (productNumbers){
        ...
        document.querySelector(".cart-number span").textContent = productNumbers + 1;
    }
    else{
...
function removeCart(code) 
    { 
            if(typeof window.localStorage[code] !== "undefined")
             ...
             let productNumbers = localStorage.getItem("cartNumbers");
             productNumbers = parseInt(productNumbers); 
             localStorage.setItem("cartNumbers", productNumbers - parseInt(window.localStorage.getItem(code)));
             document.querySelector(".cart-number span").textContent = productNumbers - parseInt(window.localStorage.getItem(code));
...
const validateName = (name) => {
  ...
  function valid() {
    ...
    if (x == 0) {
      ...
      if (checkout) {
        window.location.replace("./confrim.html");
        removeAll();
      } else {
        toast({
          title: "Error",
...
$( document ).on( "click", ".add-cart #cf", function(e) {
